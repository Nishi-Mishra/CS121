...
int main1() {
    ...
    char** map = loadLevel("example.txt", row, col, pal);  
    cout << endl << "Here? Row: " << row << "  Col: " << col << endl;
    ...
    //int col;
    //Player pal; 
    //// char** map = loadLevel("2 by 2.txt", row, col, pal); // as of now 2 by 2 txt is invalid?? ... has an exit .. ? 
    //char** map = loadLevel("example.txt", row, col, pal); 
    //resizeMap(map, row, col); 

    char h = 'H'; 
    cout << h << " : " << (char) tolower(h) << endl; 
